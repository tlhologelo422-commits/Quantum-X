...
st.caption("Gemini 3.6 Flash + Groq qwen 3.6 27b")
    ...
    if gem_key:
        try:
            ...
            for mdl in ["gemini-3.6-flash","gemini-2.5-flash","gemini-3-flash-preview","gemini-flash-latest"]:
                try:
                    resp = client.models.generate_content(model=mdl, contents=[f"You are pro scalper {market}. Analyze chart. Return ONLY JSON: {{\"trend\":\"BULLISH/BEARISH\",\"signal\":\"BUY/SELL/WAIT\",\"entry\":0,\"sl\":0,\"tp\":0,\"confidence\":85,\"reason\":\"short\"}}", img])
    ...
    if groq_key:
        try:
            ...
            for gmdl in ["qwen/qwen3.6-27b","meta-llama/llama-4-scout-17b-16e-instruct","meta-llama/llama-4-maverick-17b-128e-instruct"]:
                try:
                    comp = client.chat.completions.create(model=gmdl, messages=[{"role":"user","content":[{"type":"text","text":f"Analyze {market} chart. Return ONLY JSON {{\"trend\":\"\",\"signal\":\"BUY/SELL/WAIT\",\"entry\":0,\"sl\":0,\"tp\":0,\"confidence\":0,\"reason\":\"\"}}"},{"type":"image_url","image_url":{"url":f"data:image/jpeg;base64,{b64}"}}]}], max_tokens=500)

...
st.set_page_config(page_title="SOLID V17.2 FINAL", layout="wide")
st.title("SOLID V17.2 - FIXED SCAN + TRADE")
st.caption("Gemini 1.5 Flash FREE + Groq Vision FREE")
...
def analyze_dual_ai(image_bytes, market, gem_key, groq_key):
 ...
 if gem_key:
  try:
   ...
   for mdl in ["gemini-1.5-flash", "gemini-1.5-flash-latest", "gemini-1.5-flash-8b"]:
    try:
     model=genai.GenerativeModel(mdl)
     img=Image.open(io.BytesIO(image_bytes))
     prompt=f"You are pro scalper {market}. Analyze chart. Return ONLY JSON: {{\"trend\":\"BULLISH or BEARISH\",\"signal\":\"BUY or SELL or WAIT\",\"entry\":0,\"sl\":0,\"tp\":0,\"confidence\":85,\"reason\":\"short reason\"}}"
     resp=model.generate_content([prompt,img])
     if resp and resp.text:
      return True, resp.text, f"GEMINI {mdl}"
 ...
 if groq_key:
  try:
   ...
   for gmdl in ["llama-3.2-11b-vision-preview", "llama-3.2-90b-vision-preview"]:
    try:
     comp=client.chat.completions.create(model=gmdl,messages=[{"role":"user","content":[{"type":"text","text":f"Analyze {market} chart. Return ONLY JSON {{\"trend\":\"\",\"signal\":\"BUY/SELL/WAIT\",\"entry\":0,\"sl\":0,\"tp\":0,\"confidence\":0,\"reason\":\"\"}}"},{"type":"image_url","image_url":{"url":f"data:image/jpeg;base64,{b64}"}}]}],max_tokens=400)
     return True, comp.choices[0].message.content, f"GROQ {gmdl}"
    except Exception as e:
     groq_err=str(e)[:500]
     continue
   return False, f"Gemini err:{gem_err} | Groq err:{groq_err}", "NONE"
  except Exception as e:
   return False, f"Gemini:{gem_err} | Groq:{e}", "NONE"
 return False, f"Add FREE keys. Last Gemini err: {gem_err}", "NONE"

with st.sidebar:
 st.header("SOLID V17.2 FINAL")
   ...
   with st.spinner("Dual AI scanning..."):
    ...
    if ok:
     ...
     try:
      ...
      ent=j.get("entry"); sl=j.get("sl"); tp=j.get("tp"); conf=j.get("confidence",0)
...
def run_trade():
 ...
 if this: st.warning("Already has position")
  ...
  if ok: st.success(msg2); st.balloons()
  else: st.error(msg2)

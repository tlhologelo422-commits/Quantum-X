...
st.set_page_config(page_title="SOLID V17 DUAL AI", layout="wide")
st.title("SOLID V17 - DUAL FREE AI + ALWAYS OPENS")
st.caption("Gemini 2.0 Flash FREE + Groq FREE Vision + Direct Yahoo")
...
for k in ["con","cst","xsec","mkts","last_mkt","running","last_trade_time"]:
 if k not in st.session_state: st.session_state[k]=False if k in ["con","running"] else [] if k=="mkts" else 0 if k=="last_trade_time" else ""
...
def login():
 u,p,k,_,_=get_creds(); h={"X-IG-API-KEY":k,"Content-Type":"application/json","Accept":"application/json","Version":"2"}; b={"identifier":u,"password":p,"encryptedPassword":False}
 try:
...
def place_solid(epic,direction,size,sl=None,tp=None):
 ...
 pays=[]
 if sl and tp: pays.append({"epic":epic,"expiry":"-","direction":direction,"size":str(size),"orderType":"MARKET","currencyCode":"USD","forceOpen":True,"guaranteedStop":False,"stopLevel":str(round(float(sl),1)),"limitLevel":str(round(float(tp),1))})
...
def close_all_for_epic(epic):
 ...
 for p in this:
  po=p.get("position",{}); did=po.get("dealId"); d="SELL" if po.get("direction")=="BUY" else "BUY"; sz=po.get("size")
  url=f"{IG}/positions/otc"; pay={"dealId":did,"epic":epic,"expiry":"-","direction":d,"size":str(sz),"orderType":"MARKET"}
  try: r=requests.delete(url,headers=hdrs("2"),json=pay,timeout=15); res.append(f"{r.status_code}")
  except Exception as e: res.append(str(e))
 return res
@st.cache_data(ttl=20)
def get_yahoo_candles(sym):
 try:
  url=f"https://query1.finance.yahoo.com/v8/finance/chart/{sym}"; pr={"range":"2d","interval":"5m"}; hh={"User-Agent":"Mozilla/5.0"}
  r=requests.get(url,params=pr,headers=hh,timeout=15)
  if r.status_code==200:
   j=r.json(); result=j.get("chart",{}).get("result",[])
   if result:
    res=result[0]; ts=res.get("timestamp",[]); q=res.get("indicators",{}).get("quote",[{}])[0]; cl=q.get("close",[]); rows=[]
...
def supertrend(df,p=10,m=3.0):
 ...
 tr=pd.concat([tr1,tr2,tr3],axis=1).max(axis=1); atr=tr.rolling(p).mean(); upper=hl2+m*atr; lower=hl2-m*atr
 stl=pd.Series(index=df.index,dtype=float); direction=pd.Series(index=df.index,dtype=int)
 for i in range(len(df)):
  if i==0: stl.iloc[i]=lower.iloc[i] if not pd.isna(lower.iloc[i]) else hl2.iloc[i]; direction.iloc[i]=1
  else:
   prev=stl.iloc[i-1]
   if pd.isna(prev): prev=lower.iloc[i-1]
   if df["close"].iloc[i]<=prev: stl.iloc[i]=upper.iloc[i]; direction.iloc[i]=-1
   else: stl.iloc[i]=lower.iloc[i]; direction.iloc[i]=1
   if direction.iloc[i]==1 and stl.iloc[i]<stl.iloc[i-1]: stl.iloc[i]=stl.iloc[i-1]
   if direction.iloc[i]==-1 and stl.iloc[i]>stl.iloc[i-1]: stl.iloc[i]=stl.iloc[i-1]
 df["supertrend"]=stl; df["st_dir"]=direction; df["atr"]=atr
 return df
# END PART1
...
def analyze_dual_ai(image_bytes, market, gem_key, groq_key):
 ...
 if gem_key:
  try:
   ...
   prompt=f"""You are pro scalper {market}. Analyze chart. Return ONLY JSON: {{"trend":"BULLISH/BEARISH","signal":"BUY/SELL/WAIT","entry":0,"sl":0,"tp":0,"confidence":0,"reason":"2 lines"}}"""
   resp=model.generate_content([prompt,img])
   return True, resp.text, "GEMINI 2.0 FLASH"
  except Exception as e: gem_err=str(e)
 else: gem_err="No key"
 # 2ND AI: GROQ FREE - ULTRA FAST
 if groq_key:
  try:
   ...
   comp=client.chat.completions.create(
    ...
    messages=[{"role":"user","content":[
     {"type":"text","text":f"Analyze {market} chart. JSON only {{\"trend\":\"\",\"signal\":\"BUY/SELL/WAIT\",\"entry\":0,\"sl\":0,\"tp\":0,\"confidence\":0,\"reason\":\"\"}}"},
     {"type":"image_url","image_url":{"url":f"data:image/jpeg;base64,{b64}"}}
    ]}],
    max_tokens=400)
   return True, comp.choices[0].message.content, "GROQ LLAMA 4 SCOUT"
  except Exception as e: return False, f"Gemini:{gem_err} | Groq:{e}", "NONE"
 return False, f"Add free keys. Gemini err: {gem_err}", "NONE"

with st.sidebar:
 st.header("SOLID V17 DUAL")
...
if st.button(f"Find {mkt} EPIC",use_container_width=True):
 ...
 if ok:
  found=[]
  for x in res:
   epic=x.get("epic","") or x.get("instrument",{}).get("epic",""); name=x.get("instrumentName","") or x.get("instrument",{}).get("name","")
...
epics=[x["epic"] for x in st.session_state["mkts"]]; labs=[f"{x['epic']} | {x['name']}" for x in st.session_state["mkts"]]
...
if acc:
 try: bal=acc.get("accounts",[])[0].get("balance",{}).get("balance",0); st.metric("Balance",f"${bal}")
 except: pass
if pos:
 tot=0
 for p in pos:
  ...
  st.write(f"{p.get('market',{}).get('instrumentName','')} {po.get('direction')} {po.get('size')} PnL :{c}[${pnl}]")
 st.metric("Total PnL",f"${tot:.2f}")
else: st.write("No positions")

st.divider()
st.subheader("📸 DUAL AI SCANNER - FREE")
st.write("Upload screenshot -> 2 AIs scan, gives BUY/SELL/SL/TP")
...
if up:
 ...
 if st.button("🔍 SCAN WITH DUAL FREE AI",type="primary",use_container_width=True):
  ...
  gk=sec_gem or gem_input; gqk=sec_groq or groq_input
  if not gk and not gqk: st.error("Add Gemini FREE key from aistudio.google.com/apikey (no card needed)")
  else:
   with st.spinner("Dual AI scanning..."):
    ok,txt,model_used=analyze_dual_ai(up.getvalue(),mkt,gk,gqk)
    if ok:
     st.success(f"Done via {model_used}"); st.code(txt)
     try:
      m=re.search(r'\{.*\}',txt,re.DOTALL)
      if m: j=json.loads(m.group())
      else: j={}
      sig=j.get("signal","WAIT"); ent=j.get("entry"); sl=j.get("sl"); tp=j.get("tp"); conf=j.get("confidence",0)
      st.write(f"**{model_used}: {sig} | Conf {conf}% | Entry {ent} SL {sl} TP {tp} | {j.get('reason','')}**")
      if sig in ["BUY","SELL"]:
    ...
    except Exception as e: st.write(f"Parsed but raw: {txt} err {e}")
    else: st.error(txt)
...
if close_btn: r=close_all_for_epic(epic); st.success(f"Closed {r}"); time.sleep(1); st.rerun()
def run_trade():
